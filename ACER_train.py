...
def _train(args, T, model, shared_model, shared_average_model, optimiser, policies, Qs, Vs, actions, rewards, Qret,
           average_policies, old_policies=None,writer=None):
    ...
    writer.add_scalar("loss/policy_loss",policy_loss,T.value())
    writer.add_scalar("loss/value_loss", value_loss, T.value())
...
def train(rank, args, T,BEST, shared_model, shared_average_model, optimiser):
    torch.manual_seed(args.seed + rank)
    save_dir = os.path.join(args.name, 'checkpoint', str(rank))
    ...
    while T.value() <= args.T_max:
        ...
        while True:
            ...
            while not done:
                ...
                action_entropy = Categorical(probs=policy).entropy()
                ...
                writer.add_scalar("indicator/entropy",action_entropy, t)
                # Increment counters
                t += 1
                T.increment()
                ...
                state = next_state
            if discard:
                memory.append(state, None, None, None, discard=discard)
                break
        ...
        writer.add_scalar("reward/round_reward", round_score, T.value())
        writer.add_scalar("reward/mean_score", m_score, T.value())
        if round_score*400 > BEST.value():
            print("Save model!")
            torch.save(model.state_dict(), os.path.join(save_dir, 'model'))  # Save model params
            torch.save(memory, os.path.join(save_dir, 'memory'))  # Save memory
            torch.save((t, BEST.value()), os.path.join(save_dir, 'indicator'))  # Save data

        if m_score*400 > BEST.value():
            print("Save model!")
            torch.save(model.state_dict(), os.path.join(args.name, 'checkpoint', 'model'))  # Save model params
            torch.save(memory, os.path.join(args.name, 'checkpoint', 'memory'))  # Save memory
            torch.save((t, BEST.value()), os.path.join(args.name, 'checkpoint', 'indicator'))  # Save data
            BEST.set(int(m_score*400))
        print("Process: {}, EPISODE: {},BEST: {}, episode: {}, round_reward: {}, mean_score:{}".format(rank, T.value(),
                                                                                                       BEST.value(), t,
                                                                                              round_score, m_score))
        ...
        if not args.on_policy and len(memory) >= args.replay_start:
            ...
            print("Train the network off-policy")
            for _ in range(_poisson(args.replay_ratio)):
                ...
                _train(args, T, model, shared_model, shared_average_model, optimiser, policies, Qs, Vs,
                       actions, rewards, Qret, average_policies, old_policies=old_policies,writer=writer)
        done = True
