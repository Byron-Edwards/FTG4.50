...
from OpenAI.ACER.utils import state_to_tensor, plot_line
            ...
            for _ in range(args.evaluation_episodes):
                while True:
                    ...
                    if done:
                        ...
                        state = state_to_tensor(env.reset(p2=args.p2))
                    ...
                    # Step
                    print("Test Process Step")
                    state, reward, done, dict = env.step(action.item())
                    if dict.get('no_data_receive', False):
                        env.close()
                        # env.reset(p2=args.p2)
                        print("Test Process: {} can not receive".format(rank))
                        discard = True
                        discard_episodes += 1
                    if discard:
                        done = True
                    ...
                    if done:
                        ...
                        writer.add_scalar("reward/iteration", reward_sum, t_start)
                        avg_episode_lengths.append(episode_length)
                        break
            avg_rewards_stats = sum(avg_rewards) / (args.evaluation_episodes - discard_episodes)
            avg_episode_lengths_stats = sum(avg_episode_lengths) / (args.evaluation_episodes - discard_episodes)
            print(('[{}] Step: {:<' + l + '} Avg. Reward: {:<8} Avg. Episode Length: {:<8}').format(
                datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S,%f')[:-3],
                t_start,
                avg_rewards_stats,
                avg_episode_lengths_stats))
            fields = [t_start, avg_rewards_stats, avg_episode_lengths_stats, str(datetime.now())]

            # storing data in the dictionary.
            results_dict['t'].append(t_start)
            results_dict['reward'].append(avg_rewards_stats)
            results_dict['avg_steps'].append(avg_episode_lengths_stats)
